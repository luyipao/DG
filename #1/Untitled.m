...
for h = [1/10,1/20,1/40]
    ...
    E_h = U - U_h;
    disp(['h is ', num2str(h), '; k is: ', num2str(degree), '; error is ', num2str(sum(sum(abs(E_h)))/ (1/(h^2)))]);
end
...
%         u_hh = builder(C,X);
% end
% function [u_hh, C] = RK2(delta_t,X,C)
% [~,C1] = Euler_solve( delta_t,X,C);
% [u_temp, C_temp] = Euler_solve(delta_t,X,C1);
% C = 0.5 * (C + C_temp);
